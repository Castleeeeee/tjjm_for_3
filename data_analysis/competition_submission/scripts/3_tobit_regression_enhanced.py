...
class Tobit(GenericLikelihoodModel):
    ...
    def nloglikeobs(self, params):
        ...
        sigma = params[-1]
        if sigma <= 0:
            return np.array([1e10] * len(self.endog))
        
        mu = np.dot(self.exog, beta)
        z = (self.endog - mu) / sigma
        
        # 使用 log_ndtr 提高数值稳定性 (避免 logcdf 的下溢问题)
        prob_y_pos = norm.logpdf(z) - np.log(sigma)
        prob_y_zero = log_ndtr(-mu / sigma)
        
        ll = np.where(self.endog > 0, prob_y_pos, prob_y_zero)
        return -ll

    def fit(self, start_params=None, maxiter=10000, method='bfgs', **kwargs):
        if start_params is None:
            ols_res = sm.OLS(self.endog, self.exog).fit()
            start_params = np.append(ols_res.params, ols_res.resid.std())
        return super(Tobit, self).fit(start_params=start_params, maxiter=maxiter, method=method, **kwargs)
...
def run_regression(df, y_col, x_cols, title):
    ...
    # 检查并移除共线性的年份列（如果该子样本中该年份没有数据）
    # 或者如果 sum(dummies) == 1 且包含 const，则需要额外 drop 一个 dummy
    if 'const' in X.columns:
        year_cols_in_X = [c for c in X.columns if c.startswith('year_')]
        if year_cols_in_X:
            # 如果子样本中所有年份虚拟变量的和等于1（即没有基准年），则需要删除一个
            if np.allclose(X[year_cols_in_X].sum(axis=1), 1.0):
                print(f"检测到子样本中存在完全共线性，移除第一个年份虚拟变量: {year_cols_in_X[0]}")
                X = X.drop(columns=[year_cols_in_X[0]])
...
def main():
    ...
    # 填充缺失值 (Super-SBM 分数上限设为 2.0，DEA 效率通常在这个范围内)
    ...
    if res_base: all_results.append(("1. Baseline with Year FE", res_base))
    ...
    if res_lag: all_results.append(("2. Lagged Z1 (L.Z1)", res_lag))
    ...
    if res_dim: all_results.append(("3. Dimension Analysis (Regulation)", res_dim))

    # --- 回归 4: 异质性分析 (东部 vs 非东部) ---
    # 在异质性分析中也加入年份固定效应，增加稳健性
    res_east = run_regression(df[df['Region'] == 'East'], y_col, [f'ln_{z1_col}', f'ln_{gdp_col}', f'ln_{third_col}'] + year_cols, "异质性分析 (东部地区)")
    res_noneast = run_regression(df[df['Region'] != 'East'], y_col, [f'ln_{z1_col}', f'ln_{gdp_col}', f'ln_{third_col}'] + year_cols, "异质性分析 (非东部地区)")
    if res_east: all_results.append(("4a. Heterogeneity (East)", res_east))
    if res_noneast: all_results.append(("4b. Heterogeneity (Non-East)", res_noneast))
